...
const pkgImportPath = process.env['PKG_IMPORT_PATH'] ?? '@dizzzmas/sst-url-shortener-sdk/';
...
async function postprocess() {
  for await (const file of walk(path.resolve(__dirname, '..', '..', 'dist'))) {
    ...
    if (file.endsWith('.d.ts')) {
      ...
      // if we have `import { type Readable } from '@dizzzmas/sst-url-shortener-sdk/_shims/index'`,
