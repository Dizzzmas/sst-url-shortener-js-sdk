...
describe('instantiate client', () => {
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['SST_URL_SHORTENER_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new SstURLShortener({ baseURL: 'https://example.com', bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['SST_URL_SHORTENER_BASE_URL'] = 'https://example.com/from_env';
      const client = new SstURLShortener({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['SST_URL_SHORTENER_BASE_URL'] = ''; // empty
      const client = new SstURLShortener({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://localhost:8080/test-api');
    });

    test('blank env variable', () => {
      process.env['SST_URL_SHORTENER_BASE_URL'] = '  '; // blank
      const client = new SstURLShortener({ bearerToken: 'My Bearer Token' });
      expect(client.baseURL).toEqual('https://localhost:8080/test-api');
    });
  });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new SstURLShortener({ maxRetries: 4, bearerToken: 'My Bearer Token' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new SstURLShortener({ bearerToken: 'My Bearer Token' });
...
describe('retries', () => {
  test('retry on timeout', async () => {
    ...
    const client = new SstURLShortener({ bearerToken: 'My Bearer Token', timeout: 10, fetch: testFetch });
  ...
  test('retry count header', async () => {
    ...
    const client = new SstURLShortener({ bearerToken: 'My Bearer Token', fetch: testFetch, maxRetries: 4 });
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new SstURLShortener({ bearerToken: 'My Bearer Token', fetch: testFetch });
