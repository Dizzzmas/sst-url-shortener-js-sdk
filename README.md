...
async function main() {
  const params: SstURLShortener.URLCreateParams = { originalUrl: 'REPLACE_ME' };
  const url: SstURLShortener.URLCreateResponse = await client.urls.create(params);
}
...
```ts
async function main() {
  const url = await client.urls.create({ originalUrl: 'REPLACE_ME' }).catch(async (err) => {
    if (err instanceof SstURLShortener.APIError) {
...
await client.urls.create({ originalUrl: 'REPLACE_ME' }, {
  maxRetries: 5,
});
```
...
const response = await client.urls.create({ originalUrl: 'REPLACE_ME' }).asResponse();
console.log(response.headers.get('X-My-Header'));
console.log(response.statusText); // access the underlying Response object

const { data: url, response: raw } = await client.urls.create({ originalUrl: 'REPLACE_ME' }).withResponse();
...
add the following import before your first import `from "SstURLShortener"`:
